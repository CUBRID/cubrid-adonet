    ...
    public class UnitTestCases
    {
        ...
        public void Test_CreateFunction()
        {
            using (CUBRIDConnection conn = new CUBRIDConnection())
            {
                ...
                try
                {
                    UnitTestCases.ExecuteSQL("drop function hello", conn);
                }
                catch { }

                string sql = "CREATE FUNCTION hello() RETURN string AS LANGUAGE JAVA NAME 'SpCubrid.HelloCubrid() return java.lang.String'";
                using (CUBRIDCommand cmd = new CUBRIDCommand(sql, conn))
                {
                    cmd.ExecuteNonQuery();
                }
