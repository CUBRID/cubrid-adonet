    ...
    public class UnitTestCases
    {
        ...
        public void Test_DataSet_Basic()
        {
            using (CUBRIDConnection conn = new CUBRIDConnection())
            {
                ...
                using (CUBRIDDataAdapter da = new CUBRIDDataAdapter())
                {
                    ...
                    Assert.IsTrue(dt0.DefaultView.Count == 215);
        ...
        public void Test_ConnectionProperties()
        {
            using (CUBRIDConnection conn = new CUBRIDConnection())
            {
                ...
                Assert.AreEqual(conn.ConnectionTimeout, 30);
                Assert.AreEqual(conn.CurrentDatabase(), "demodb");
                Assert.AreEqual(conn.Database, "demodb");
                Assert.AreEqual(conn.DBVersion, "8.4.3.1005");
                Assert.AreEqual(conn.DataSource, "10.34.64.122");
                Assert.AreEqual(conn.AutoCommit, true);
                Assert.AreEqual(conn.LockTimeout, 30);
                Assert.AreEqual(conn.ConnectionTimeout, 30);
                Assert.AreEqual(conn.IsolationLevel, CUBRIDIsolationLevel.TRAN_REP_CLASS_UNCOMMIT_INSTANCE);
        ...
        public void Test_GetTableNameFromOid()
        {
            using (CUBRIDConnection conn = new CUBRIDConnection())
            {
                ...
                Assert.IsTrue(tableName == "history");
            }
        }
